...
cv::Point mouseLeftButenClkCordinate;
cv::Mat extendBorderAndConcatenateColorMap(const cv::Mat& src)
{
	cv::Mat colorMap = cv::imread("C:/experimental photos/colorMap.png");
	cv::resize(colorMap, colorMap, cv::Size(), 2, 2);
	cv::Mat image(src.rows + colorMap.rows + 2, src.cols, src.type());
	cv::Rect WhereRec(0, 0, src.cols, src.rows);
	src.copyTo(image(WhereRec));
	cv::Rect WhereRecColorMap(0, src.rows + 2, colorMap.cols, colorMap.rows);

	colorMap.copyTo(image(WhereRecColorMap));
	cv::Point p(86, image.rows - 21);
	double min = cv::norm(image.at<cv::Vec3b>(image.rows - 21, 86) - image.at<cv::Vec3b>(mouseLeftButenClkCordinate.y, mouseLeftButenClkCordinate.x));

	for (int i = 86; i < 459; ++i)
	{
		auto dist = cv::norm(image.at<cv::Vec3b>(image.rows - 21, i) - image.at<cv::Vec3b>(mouseLeftButenClkCordinate.y, mouseLeftButenClkCordinate.x));
	...
	cv::line(image, cv::Point(p.x, image.rows - 44), cv::Point(p.x, image.rows), cv::Scalar(0, 0, 0), 2);
	double gradus = -24.0 + (/*459 -*/ p.x - 86) * 0.07;
	cv::putText(image, std::to_string(gradus), mouseLeftButenClkCordinate, cv::FONT_HERSHEY_COMPLEX_SMALL, 1, cv::Scalar(0, 0, 0));
...
void callback_mouse_click(int event, int x, int y, int flags, void* user_data)
{
	...
	else if (event == cv::EVENT_LBUTTONDOWN)
	{
		...
		x_start = x;
		y_start = y;
		mouseLeftButenClkCordinate.x = x;
		mouseLeftButenClkCordinate.y = y;
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		std::string train_filename = images_path + "\\train.txt";
		...
		std::cout << "Image path : " << images_path_cv << std::endl;
		...
		std::cout << "FILES IN FOLDER : " << filenames_in_folder_cv.size() << std::endl;
		...
		std::string const window_name = "Improvis Annotation Tool [Forked from YOLO_MARK]";
		namedWindow(window_name, WINDOW_NORMAL);
		resizeWindow(window_name, 1280, 720);
		//cv::Mat withColorMap = extendBorderAndConcatenateColorMap(frame);
		...
		do {
			...
			cv::Mat withColorMap = extendBorderAndConcatenateColorMap(frame);
			imshow(window_name, withColorMap);
			//imshow("frame", frame);
